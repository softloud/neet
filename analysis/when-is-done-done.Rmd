...
The `covr::` function `package_coverage` provides the percentage of the lines of code run in tests and is a succinct method to get an at-a-glance sense coverage of testing scripts. But this assumes we have considered everything that might require testing. Whilst this is informative, it's not comprehensive. A 100% in all functions in `covr::`, we do not consider done. 

An excellent way to arrive at *done* for an algorithm is to define, even if only broadly, what *done* looks like *before* we set out. We define what `code::proof` is requrired.
...
For those comfortable with *graphs* understood as mathematical objects as a set of vertices from $V$, and a set of edges $V \times V$. There are visualisation options where the vertices can be tagged with attributes.  
...
workflow_segments <- tiles %>%
  slice(-1) %>% # take the first row off
  bind_rows(tiles[nrow(tiles),]) %>%  # add last row again
...
This section describees a 

## one `neet` test per fucntion 
