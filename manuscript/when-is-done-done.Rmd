...
In the following code example, we provide some of the `neet` tests associated with the `aggreCAT::` package. The methods have been replaced with `< n >`, as a condition of this research is to keep the other teams blinded to the aggregation calculations. Once the project is completed, the software and analyses will be open source. 
...
test_neet(aggregate_cs(expert_judgements_comp_subset), "data.frame")

# methods -----------------------------------------------------------------

test_neet(< 1 > (expert_judgements_comp_subset), "data.frame")
test_neet(< 2 > (expert_judgements_comp_subset), "data.frame")
test_neet(< 3 > (expert_judgements_comp_subset), "data.frame")
